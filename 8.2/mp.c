...
void check_and_print_result(int*);

int main() {
    pid_t pid;
    int result[100] = {0}; // initialize all values to 
    char ch = 'y';
    int i = 0;
    
    while(1) {
        // check for completed child processes
        check_and_print_result(result);
...
void calc(int a, int b, int c, int d) {
    if (pow(a,b) < pow(c,d)) // corrected comparison between c^b and c^d
        exit(1);
...
void check_and_print_result(int *result) {
    int status;
    pid_t child_pid;
